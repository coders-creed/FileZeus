...
(* in-built modules *)
	...
	(* optional, used to keep index string length uniform *)
	...
	(*hashes a file with a basis hahs function*)
	...
	(* not used in the current setting *)
	...
	(* returns a list of members of list between start and end indices; helper function *)
	...
	(* extracts hash value of root of the tree *)
	...
	(* builds the merkle tree (non-padded) *)
	let rec build_merkle filelist basis_hash child_id prefix index_file=
		...
		1 -> let r = 
			...
			| filer -> File.append_file_break index_file (filer^";"^prefix); ()) in 
